...
class DefaultSuggestionEngineTest {
    ...
    // An empty user dictionary: these tests exercise provider fan-out, not learned words.
    private val userDictionary = com.bornomala.keyboard.suggestions.data.local.UserDictionaryRepository(
        com.bornomala.keyboard.suggestions.util.lazyOf(com.bornomala.keyboard.suggestions.util.FakeUserDictionaryDao()),
        dispatchers,
    )

    // An empty phonetic index is fine for these English-only tests.
    private val phonetic = com.bornomala.keyboard.suggestions.data.dictionary.BanglaPhoneticRepository(
        com.bornomala.keyboard.suggestions.util.InMemoryDictionarySource(emptyMap()),
        dispatchers,
    )
    ...
    fun `unavailable providers are never queried`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(cloud), dispatchers, phonetic, userDictionary)
    ...
    fun `only available providers contribute`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(available, unavailable), dispatchers, phonetic, userDictionary)
    ...
    fun `merges across providers and ranks by score`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(p1, p2), dispatchers, phonetic, userDictionary)
    ...
    fun `duplicate words keep the highest score`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(high, low), dispatchers, phonetic, userDictionary)
    ...
    fun `provider failure is dropped and others survive`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(failing, ok), dispatchers, phonetic, userDictionary)
    ...
    fun `honours the limit after merge`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(p), dispatchers, phonetic, userDictionary)
    ...
    fun `empty provider set yields no suggestions`() = runTest {
        val engine = DefaultSuggestionEngine(emptySet(), dispatchers, phonetic, userDictionary)

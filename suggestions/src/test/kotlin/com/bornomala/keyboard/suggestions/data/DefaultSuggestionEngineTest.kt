...
class DefaultSuggestionEngineTest {
    ...
    fun `unavailable providers are never queried`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(cloud), dispatchers, phonetic)
    ...
    fun `only available providers contribute`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(available, unavailable), dispatchers, phonetic)
    ...
    fun `merges across providers and ranks by score`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(p1, p2), dispatchers, phonetic)
    ...
    fun `duplicate words keep the highest score`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(high, low), dispatchers, phonetic)
    ...
    fun `provider failure is dropped and others survive`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(failing, ok), dispatchers, phonetic)
    ...
    fun `honours the limit after merge`() = runTest {
        ...
        val engine = DefaultSuggestionEngine(setOf(p), dispatchers, phonetic)
    ...
    fun `empty provider set yields no suggestions`() = runTest {
        val engine = DefaultSuggestionEngine(emptySet(), dispatchers, phonetic)
